...
public class ApiService {
    ...
    public void executeOnStartup() {
        // Step 1: Call generateWebhook endpoint
        GenerateWebhookResponse response = callGenerateWebhook();

        if (response != null) {
            // Step 2: Solve the problem based on regNo
            List<List<Integer>> outcome = solveProblem(response);

            // Log calculated outcome for debugging
            System.out.println("Calculated outcome: " + outcome);

            // Step 3: Prepare the test webhook request
            ...
            // Step 4: Send result to webhook with retry
    ...
    private List<List<Integer>> solveProblem(GenerateWebhookResponse response) {
        ...
        // Log usersWrapper to see the structure of the data
        ...
        // Log the list of users to ensure correct mapping
        System.out.println("Mapped Users: " + users);
    
        // Logic based on regNo
    ...
    public List<List<Integer>> findNthLevelFollowers(List<User> users, int findId, int n) {
        ...
        // Collect followers at nth level (this could involve recursion or iteration)
        List<List<Integer>> nthLevelFollowers = new ArrayList<>();

        // For simplicity, we'll return the direct followers of the user as the nth level
        if (n == 1) {
            nthLevelFollowers.add(user.getFollows());
        }

        // You could expand this logic for deeper levels of following
    ...
    public List<List<Integer>> findMutualFollowers(List<User> users) {
        ...
        // Log the follow map for debugging
    ...
    private void sendToWebhookWithRetry(String webhookUrl, String accessToken, TestWebhookRequest request) {
        ...
        while (attempt < maxRetries && !success) {
            attempt++;
            try {
                System.out.println("Sending to webhook: " + request); // Log the content being sent to the webhook
...
//     //     // Implement your nth-level followers logic here
